...
#if CONFIG_USE_EMOTE_STYLE
LV_FONT_DECLARE(font_puhui_20_4_basic);
#else
...
private:
...
#if CONFIG_USE_EMOTE_STYLE
            // 测试功能：循环播放表情ID
            static int aaf_id = MMAP_EMOJI_SMALL_HAPPY_EAF;
            if(aaf_id > MMAP_EMOJI_SMALL_WINKING_EAF) {
                aaf_id = MMAP_EMOJI_SMALL_HAPPY_EAF;
            }
            TestSetEye(aaf_id);
            aaf_id++;
#else
    ...
    void InitializeLcdDisplay()
    {
        ...
        display_ = new anim::SPIEmoteDisplay(panel_io, panel, DISPLAY_WIDTH, DISPLAY_HEIGHT, {
            ...
            .basic_font = &font_puhui_20_4_basic,
